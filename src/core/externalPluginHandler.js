...
function extractCardProgress(content, foil) {
  ...
  const formattedOwned = Number(owned.replace(/ /g, '')).toLocaleString('en-US');
  const formattedTotal = Number(total.replace(/ /g, '')).toLocaleString('en-US');
...
function externalPluginHandler(msgMap, playerName, content, extra, URL) {
  const { cardName, newForCollection, foil } = extra.metadata;
